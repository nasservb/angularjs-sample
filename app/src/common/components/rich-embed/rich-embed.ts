...
export class MindsRichEmbed {
  ...
  parseInlineEmbed(current?: any) {
    ...
    if ((matches = vimeo.exec(url)) !== null) {
      ...
      // Aparat
      let aparat  = /^(?:https?:\/\/)?(?:www\.)?aparat\.com\/v\/([A-Za-z0-9]+)/;
      if ((matches = aparat.exec(url)) !== null) {
          if (matches[1]) {
              return {
                  id: `video-aparat-${matches[1]}`,
                  className: 'm-rich-embed-video m-rich-embed-video-iframe m-rich-embed-video-aparat',
                  html: this.sanitizer.bypassSecurityTrustHtml(`<iframe src="https://www.aparat.com/video/video/embed/videohash/${matches[1]}/vt/frame" allowFullScreen="true" webkitallowfullscreen="true" mozallowfullscreen="true" ></iframe>`),
                  playable: true
              };
          }
      }
