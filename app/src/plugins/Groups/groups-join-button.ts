...
@Component({
  selector: 'afs-groups-join-button',

  inputs: ['_group: group'],
  outputs: ['membership'],
  template: `
    <button class="afs-group-join-button"
    ...
    <span *ngIf="group['is:invited'] &amp;&amp; !group['is:member']">
      <button class="afs-group-join-button" (click)="accept()" i18n>Accept</button>
      <button class="afs-group-join-button" (click)="decline()" i18n>Decline</button>
    </span>
    <button class="afs-group-join-button subscribed " *ngIf="group['is:member']" (click)="leave()" i18n>Leave</button>
    <button class="afs-group-join-button" *ngIf="group['is:awaiting']" (click)="cancelRequest()" i18n>Cancel request</button>
