...
@Component({
  selector: 'afs-button-thumbs-down',
  inputs: ['_object: object'],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <a class="mdl-color-text--blue-grey-500" (click)="thumb()" [ngClass]="{'selected': has() }">
      <i class="material-icons">thumb_down</i>
      <span class="afs-counter" *ngIf="object['thumbs:down:count'] > 0">{{object['thumbs:down:count'] | number}}</span>
    </a>
  `
})
...
export class ThumbsDownButton {
  ...
  thumb() {
    ...
    {
		alert("متاسفانه این امکان  برای کاربرانی که پروفایل خود را عمومی نکرده اند وجود ندارد . ");      
		return false ; 
	}
