...
@Component({
  selector: 'afs-button-user-dropdown',
  inputs: ['user'],
  outputs: ['userChanged'],
  template: `
    <button class="material-icons" (click)="toggleMenu($event)">settings</button>

    <ul class="afs-dropdown-menu" [hidden]="!showMenu" >
      <li class="mdl-menu__item" [hidden]="user.blocked" (click)="block()" i18n>مسدود کردن  @{{user.username}}</li>
      <li class="mdl-menu__item" [hidden]="!user.blocked" (click)="unBlock()" i18n>رفع مسدودیت @{{user.username}}</li>
      <li class="mdl-menu__item" [hidden]="!user.subscribed" (click)="unSubscribe()" i18n>دنبال نکردن</li>
      ...
      <li class="mdl-menu__item" *ngIf="session.isAdmin()" [hidden]="user.banned !== 'yes'" (click)="unBan()" i18n>رفع ممنوعیت کلی</li>
      <li class="mdl-menu__item"
        *ngIf="session.isAdmin()"
        [hidden]="user.spam"
    ...
    <div class="afs-bg-overlay" (click)="toggleMenu($event)" [hidden]="!showMenu"></div>
    ...
    >
        <p confirm-message>
            آیا شما اطمینان دارید که میخواهید پروفایل این کاربر را مسدود نمایید
            <br>
        </p>
