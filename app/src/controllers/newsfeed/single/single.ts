    ...
    this.client.get('api/v1/newsfeed/single/' + guid, {}, { cache: true })
      .then((data: any) => {
        ...
        switch (this.activity.subtype) {
          ...
          case 'service':
            this.router.navigate(['/service/view', this.activity.guid], { replaceUrl: true });
            break;  
        }
      })
      .catch(e => {
        if (e.status === 0) {
          this.error = 'با عرض پوزش، یک خطایی در اجرای برنامه رخ داده است.';
        } else {
          this.error = 'با عرض پوزش، محتوای درخواستی وجود ندارد';
        }
