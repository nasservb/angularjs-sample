...
export class Channel {
  ...
  load() {
    ...
    this.client.get('api/v1/channel/' + this.username, {})
      ...
      .catch((e) => {
        if (e.status === 0) {
          this.error = 'با عرض پوزش, خطایی در برقراری ارتباط با سرور رخ داد.';
        } else {
  ...
  upload_avatar(file) {
    ...
    this.upload.post('api/v1/channel/avatar', [file], { filekey: 'file' })
      .then((response: any) => {
        ...
        window.Afs.user.icontime = Date.now();
      });
  }
  ...
  setCity(row: any) {
    ...
    this.user.city = window.Afs.user.city;
  ...
  canDeactivate() {
    ...
    if (progress > 0 && progress < 100) {
      return confirm('فایل شما در حال آپلود است . آیا مطمئن هستید ؟ ');
    }
